...
int main(int argc, char** argv) {
    ...
    while (i < argc) {
        if (!strcmp(argv[i], "-basepath")) {
            ...
            if (!strcmp(argv[i + 1], "MINIMAL")) {
                routing_strategy = DragonflySwitch::MINIMAL;
            } else if (!strcmp(argv[i + 1], "VALIANT")) {
                routing_strategy = DragonflySwitch::VALIANT;
            } else if (!strcmp(argv[i + 1], "UGAL_L")) {
                routing_strategy = DragonflySwitch::UGAL_L;
            } else if (!strcmp(argv[i + 1], "SOURCE")) {
                routing_strategy = DragonflySwitch::SOURCE;
            } else {
    ...
    if (ecn) {
        ...
        DragonflyTopology::set_ecn(ecn, ecn_low, ecn_high);
    }
