...
def build_output_filename(experiment_name, tag, args, p, a, h):
    return f"{experiment_name}_p{p}a{a}h{h}_{args.cc}_{tag}.tmp"


def parse_args():
    parser = argparse.ArgumentParser(description="Run dragonfly simulations (optional parallelism and single experiment selection).")
...
# New: run a single variant (routing or routing+LB) in its own temp workspace
def run_variant(task):
    ...
    with tempfile.TemporaryDirectory(prefix="df_variant_") as workdir:
        ...
        log_path = os.path.join(run_logs_dir, build_output_filename(experiment_name, tag, args, p, a, h))
...
if __name__ == '__main__':
    ...
    p, a, h = (4, 8, 4)
    topo = f"p{p}a{a}h{h}"
    input_dir = f"experiments/df/{topo}"
    output_dir = os.path.join(args.output_root, "df", topo, "no_fail")
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
    print(f"Repo root: {repo_root}")
    binary_path = os.path.join(repo_root, "htsim/sim/datacenter/htsim_uec_df")
    basepath_abs = os.path.abspath(os.path.join(repo_root, "htsim/sim/datacenter/topologies", "dragonfly", topo))
